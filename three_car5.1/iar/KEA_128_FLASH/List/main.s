...
//    List file    =  
//        H:\智能车程序2018\电磁三轮——ZS\5.1\iar\KEA_128_FLASH\List\main.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   69 
//   70     //清除中断标志   
//   71     if ((++PIT_times) >= 200)//每 500ms LED0翻转一次
        LDR      R0,??DataTable1_3
        LDRH     R0,[R0, #+0]
        ADDS     R0,R0,#+1
        LDR      R1,??DataTable1_3
...
//  115           UART_SendFloat(gyr_angle_speed,1);  
        MOVS     R1,#+1
        LDR      R0,??DataTable1_7
        LDR      R0,[R0, #+0]
        BL       UART_SendFloat
...
//  117 }  
//  118  
//  119 //刷OLED，使用了硬件spi速率已经缩小到了us级别
//  120     if (KEY_sta)
...
//  121     {
//  122         if (KEY_menuChange)//菜单选择和数据显示 更新分两次进行 减小数据量
        LDR      R0,??DataTable1_9
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
        BEQ      ??PIT_CH0_IRQHandler_4
//  123         {
//  124             OLED_MenuFlicker();
        BL       OLED_MenuFlicker
//  125             KEY_menuChange = !KEY_menuChange;
        LDR      R0,??DataTable1_9
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
...
??PIT_CH0_IRQHandler_5:
        ...
        STRB     R0,[R1, #+0]
//  126         }
//  127         KEY_ParaMenuDisplay();//大概需要1ms左右,但由于第二次按键不操作，所以时间差可以忽略
...
//  136 
//  137 
//  138 //uint8 scope_send_num;
//  139 //uint8 scope_send_count=0;
//  140 //uint8 send_flag=0;
//  141 ////系统
//  142 
//  143 //uint16 Runing_times=0;
//  144 //uint16 Runing_Distance = 0;
//  145 //#define Runing_Furthest  3000		//试车最远距离(单位:cm)
//  146 //uint8 Car_Status = Ready;	//开机即直立
//  147 //uint8 Stop_Car_cnt = 0;
//  148 //uint8 Stop_Car_flag=0;
//  149 //uint8 forward_flag = 0;
//  150 //float power;
//  151 //uint8 key=0;
//  152 //
//  153 //uint8 alarm_status=0;
//  154 ////速度
//  155 //uint8 speed_control_period = 0;
//  156 //uint8 podao_spd_flag=0;
//  157 //uint8 podao_flag;/* 当赛道识别程序加入后，将此定义移到该文档 --- han 2017-11-1 */
//  158 ////方向
//  159 //uint32 Brick_Time = 0,Podao_Time = 0,Start_Line_Time = 0,Test_Time = 0;
//  160 //uint8 boma, boma_last = 0, key_value;
// 
