...
void usmart_init(uint8 sysclk);//初始化
uint8 usmart_cmd_rec(uint8*str);	//识别
void usmart_exe(void);		//执行
void usmart_scan(void);     //扫描
uint32 read_addr(uint32 addr);	//读取指定地址的值
void write_addr(uint32 addr,uint32 val);//在指定地址写入指定的值
