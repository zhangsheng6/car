...
//*str2:字符串2指针
//返回值:0，相等;1，不相等;
uint8 usmart_strcmp(uint8 *str1,uint8 *str2)
{
	while(1)
	{
...
//*str1:字符串1指针
//*str2:字符串2指针
void usmart_strcopy(uint8*str1,uint8 *str2)
{
	while(1)
	{
		*str2=*str1;	//拷贝
...
//m^n函数
//返回值:m^n次方
uint32 usmart_pow(uint8 m,uint8 n)
{
	uint32 result=1;
	while(n--)result*=m;
	return result;
...
uint8 usmart_str2num(uint8*str,uint32 *res)
{
	uint32 t;
	int tnum;
	uint8 bnum=0;		//数字的位数
	uint8 *p;
	uint8 hexdec=10;	//默认为十进制数据
	uint8 flag=0;		//0,没有符号标记;1,表示正数;2,表示负数.
	p=str;
	*res=0;//清零.
	while(1)
	{
...
//返回值:0,成功;其他,失败.
uint8 usmart_get_cmdname(uint8*str,uint8*cmdname,uint8 *nlen,uint8 maxlen)
{
	*nlen=0;
...
//str:字符串指针
//返回值:下一个字符
uint8 usmart_search_nextc(uint8* str)
{
	str++;
	while(*str==' '&&str!='\0')str++;
	return *str;
...
//返回值:0,成功;其他,错误代码.
uint8 usmart_get_fname(uint8*str,uint8*fname,uint8 *pnum,uint8 *rval)
{
	uint8 res;
	uint8 fover=0;	  //括号深度
	uint8 *strtemp;
	...
	uint8 parmnum=0;
	uint8 temp=1;
	uint8 fpname[6];//void+X+'/0'
	uint8 fplcnt=0; //第一个参数的长度计数器
	uint8 pcnt=0;	 //参数计数器
	uint8 nchar;
	//判断函数是否有返回值
	strtemp=str;
...
uint8 usmart_get_aparm(uint8 *str,uint8 *fparm,uint8 *ptype)
{
	uint8 i=0;
	uint8 enout=0;
	uint8 type=0;//默认是数字
	uint8 string=0; //标记str是否正在读
	while(1)
	{
...
//num:第num个参数,范围0~9.
//返回值:该参数的起始地址
uint8 usmart_get_parmpos(uint8 num)
{
	uint8 temp=0;
	uint8 i;
...
//返回值:0,成功;其他,错误代码.
uint8 usmart_get_fparam(uint8*str,uint8 *parn)
{
	uint8 i,type;
	uint32 res;
	uint8 n=0;
	uint8 len;
	uint8 tstr[PARM_LEN+1];//字节长度的缓存,最多可以存放PARM_LEN个字符的字符串
		...
		switch(type)
		{
			case 0:	//数字
				if(tstr[0]!='\0')				//接收到的参数有效
				{
					...
					*(uint32*)(usmart_dev.parm+usmart_get_parmpos(n))=res;//记录转换成功的结果.
