...
static void LPLD_SDHC_InitGPIO(uint32 init)
{
  ...
  PORTE_PCR2 = init & (PORT_PCR_MUX(4) | PORT_PCR_PS_MASK | PORT_PCR_PE_MASK | PORT_PCR_DSE_MASK);    // SDHC.CLK                                        
...
//    FALSE--停止运行
//------------------------------------------------------------------------------
static uint8 LPLD_SDHC_IsRunning(void)
{
...
static DRESULT LPLD_SDHC_SendCommand(ESDHC_COMMAND_STRUCT_PTR command)
{
  ...
  while ((SDHC_BASE_PTR->PRSSTAT & SDHC_PRSSTAT_CIHB_MASK) || n < 1000 );
 //  Led_On(5);
  //初始化命令
...
{  
  ...
  case IO_IOCTL_ESDHC_INIT:  
    //初始化SDHC模块
    result = LPLD_SDHC_Init (core_clk_mhz*1000, 400000);
    if (RES_OK != result)
    {
      break;
    }
    ...
    //设置SDHC工作状态下的默认波特率
    LPLD_SDHC_SetBaudrate (core_clk_mhz*1000, 25000000);
  ...
  case IO_IOCTL_ESDHC_GET_BAUDRATE:
    ...
    else
    {
      ...
      *param32_ptr = (uint32)((core_clk_mhz*1000 / val));
    }
    break;
  case IO_IOCTL_ESDHC_SET_BAUDRATE:
    ...
    else
    {
      if (! LPLD_SDHC_IsRunning ())
      {
        ...
        //设置波特率
        LPLD_SDHC_SetBaudrate (core_clk_mhz*1000, *param32_ptr);
...
{ 
  ...
  {   
    ...
    while (remains >= 4)
    { 
      if (SDHC_BASE_PTR->IRQSTAT & (SDHC_IRQSTAT_DEBE_MASK | SDHC_IRQSTAT_DCE_MASK | SDHC_IRQSTAT_DTOE_MASK))//数据数据位错误 | 数据 CRC 错误 | 数据超时错误
      {
      ...
      do
      { m++; }
      while (((0 == (SDHC_BASE_PTR->PRSSTAT & SDHC_PRSSTAT_BREN_MASK)) && (SDHC_BASE_PTR->PRSSTAT & SDHC_PRSSTAT_DLA_MASK)) || m < 1000);
