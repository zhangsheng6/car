...
;
        ...
        ;MODULE 控制指令是用来标记 modules 源码的开始和结束，
        ;后边的 ?cstartup 是模块的名字，此文档的最后的 END 表明模块的结束
        MODULE  ?cstartup
        ...
        EXTERN  __iar_program_start ;EXTERN 用导入其他模块的 symbol（符号）
        EXTERN  Start
        PUBLIC  __vector_table      ;PUBLIC 导出 symbol（符号）
        PUBLIC  __Vectors
        PUBLIC  __Vectors_End
        PUBLIC  __Vectors_Size

        ;DATA 表示下边中的标签是 32 位的标签
        ;DCD 是数据定义或者 重定位指令
        ;sfe 指令作用是返回栈的结尾
        ;顺序一定不能变,中断发生时会根据向量表的首地址和偏移量来找到程序的入口
        DATA
__vector_table
        DCD     sfe(CSTACK)               ; c 程序的栈的栈顶地址
        DCD     Reset_Handler             ; 向量表的开始地址
...
;;
        ;THUMB 表明下边是 thumb 指令
        ;此处的 __iar_program_start 已经被封装到了 IAR 自带的C库启动代码中了，
        ;当我们编译的时候，在项目属性的 linker，library中勾选了 Automatic runtime library ，
        ;就告诉了编译器用库中的 __iar_program_start ，具体实现了什么，
        ;我们可以查看 IAR 工具为我们提供的源码，具体路径在 IAR 安装目录下的 arm\src\lib\thumb
        THUMB

        PUBWEAK Reset_Handler                ;Reset_Handler 在开机或者复位的时候执行
        SECTION .text:CODE:REORDER:NOROOT(2) ;声明
Reset_Handler
        LDR     R0, =Start                    ;跳转到 Start 函数
        BLX     R0
        LDR     R0, =__iar_program_start      ;跳转到 __iar_program_start 函数
        BX      R0
