...
float gyr_angle_speed,gyr_angle_speed_up,//陀螺仪角速度
      gyr_angle_speed_static=3,//陀螺仪静止时的角速度
...
float gyr_offset_z =21.24,gyr_offset_z_sum=0;//陀螺仪z轴偏移量
int   gg=0;
...
//陀螺仪Z,Y轴数据读取
  L3G4200_Read();
//  if(++sure_time<101)//2.5*100=250ms
//    gyr_offset_z_sum+=gyro_z;
//  else
//    offset_flag=1;
//  
//  if(offset_flag==1)
//  {
//      gyr_offset_z=gyr_offset_z_sum/100;
//      offset_flag=2;
//  }
...
//陀螺仪角速度静态滤波，代替加速度计
  if(abs_f(gyr_angle_speed)<gyr_angle_speed_static)
  {
    gyr_angle_speed=0;
  }
/* 1 先验估计 -----------------------------------------------------------  */
